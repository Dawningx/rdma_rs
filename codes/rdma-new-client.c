...
int main(int argc, char **argv)
{
    ...
    if (ret) {
        fprintf(stderr, "Failed to resolve the address\n");
        ret = 1;
        goto destroy_resources;
    }
    ...
    if (!ec) {
        fprintf(stderr, "Failed in rdma_create_event_channel()\n");
        ret = 1;
        goto destroy_resources;
    }
    ...
    if (ret) {
        fprintf(stderr, "Failed in rdma_create_id()\n");
        ret = 1;
        goto destroy_resources;
    }
    ...
    if (ret) {
        fprintf(stderr, "Failed in rdma_resolve_addr()\n");
        ret = 1;
        goto destroy_resources;
    }
...
int on_connection(void *context)
{
  ...
  // Here we can add some condition sentence which decides when to send the message
  char message[BUFFER_SIZE];
  memset(message, 0, BUFFER_SIZE);
  get_message(message);
  snprintf(conn->send_region, BUFFER_SIZE, message);
