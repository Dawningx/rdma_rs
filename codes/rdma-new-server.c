...
void on_completion(struct ibv_wc *wc)
{
  ...
  if (wc->opcode & IBV_WC_RECV) {
    ...
    /* Ensure that global_ctx is not null */
    while (!global_ctx)
      ;
    struct connection *conn_ = (struct connection *)global_ctx;
...
void send_message(struct connection *conn) {
    struct ibv_send_wr wr, *bad_wr = (struct ibv_send_wr *)malloc(sizeof(struct ibv_send_wr));
    struct ibv_sge sge;
...
void send_string(struct ibv_cq *cq, char *buffer) {
